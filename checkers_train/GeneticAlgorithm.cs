    ...
    private void playAllPercep()
    {
        ...
        for (int i=0; i<sizeP; i++)
            for(int j=0; j<5;  j++)
            {
                ...
                { 
                    id = rnd.Next(0, sizeP); 
                    if((i==29)||(population[29].countPlay2 < 5))
                    {
                        break;
                    }
                }
                while ((id == i)||(population[id].countPlay2>=5));
                playPercep(population[i], population[id]);
                population[i].coutCheckers = 12;
                population[id].coutCheckers = 12;
                population[id].countPlay2 += 1;
            }
